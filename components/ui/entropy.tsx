...
/**
 * Order ↔ chaos particle field.
 * Left = chaos (ideas), right = order (software)   matches the brand motif.
 */
export function Entropy({
  className = "",
  size = 400,
  theme = "light",
}: EntropyProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const particleColor = theme === "light" ? "#0A0A0A" : "#FFFFFF";
    ...
    class Particle {
      ...
      update(): void {
        ...
        // Chaos lives on the LEFT half (ideas)
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        ...
        // Right half = order (software); left half = chaos (ideas)
